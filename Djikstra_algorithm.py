...
# Visualisasi graf dengan warna start/goal
def visualize_graph(graph, path=[], start=None, goal=None):
    ...
    # Node Colors: Default biru muda, start biru tua, goal merah
    node_colors = []
    for node in G.nodes:
        if node == start:
...
def find_path():
    ...
    try:
        ...
        # Format rute menjadi teks multiline, 4 kota per baris
